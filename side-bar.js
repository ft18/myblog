 ...
 */
$(function () {
    $(".float-menu>ul>li>a").mouseenter(function () {
        $(this).find(".a-d").stop().animate({"top":"-55"},500);//鼠标进入li，a标签上翻

    }) ;
    $(".float-menu>ul>li>a").mouseleave(function () {
    ...
    $(".float-menu .fl ").mouseout(function () {
        ...
        $('.float-box>dl').stop().animate({"right":"-55","top":"0","width":"50"},500);
    ...
    $(document).click(function(){
        $('.float-box>dl').stop().animate({"right":"-55","width":"50"},1000).find("dt").removeClass("current");//点击页面时dl恢复位置,dt清除选中
        $(".fl").mouseout(function () {//恢复鼠标离开事件
            $('.float-box>dl').stop().animate({"right":"-55","width":"50"},1000);
        });
    });
